...
**发X笔记（带配图）** - 使用 Nano Banana API：
1. 调用 Nano Banana API 生成图片（需要API Key）
2. 下载生成的图片到本地
3. 打开X发布页面
4. 上传生成的图片
5. 输入文字内容
...
| "帮我发一条X并配上图片" | 调用Nano Banana API生成图片 → 上传图片 → 输入文字 → 发布 |
| "帮我发一条X并配上 xxx 主题的图片" | 理解图片主题 → 调用API生成图片 → 上传 → 输入文字 → 发布 |
...
Nano Banana API 是一个AI图片生成API，可以根据文字描述生成图片。

**如何使用：**

1. **配置 API Key**：告诉 Claude 你的 Nano Banana API Key
2. **生成图片**：让 Claude 根据描述生成图片
3. **自动发布**：生成的图片会自动上传到X并发布

**使用示例：**

| 你对Claude说 | Claude会执行 |
|-------------|-------------|
| "帮我发一条X，配一张科技感的图片" | 调用API生成科技感图片 → 上传到X → 发布 |
| "发一条X，配一张可爱猫咪的图片" | 调用API生成可爱猫咪图片 → 上传到X → 发布 |
| "发X，配一张未来城市" | 调用API生成未来城市图片 → 上传到X → 发布 |
